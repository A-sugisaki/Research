...
class ESN():
    ...
    def __init__(self, n_inputs, n_outputs, n_reservoir=200,
                 ...
                 learning_rate=0.5, input_shift=None,
    ...
    def fit(self, inputs, outputs, learning_rate, inspect=False):
        """
        ...
        # Solve for W_out:
        if (self.W_out == self.first).all():
            self.W_out = np.dot(np.linalg.pinv(extended_states[transient:, :]), self.inverse_out_activation(teachers_scaled[transient:, :])).T
        else:
            self.W_out = (1 - self.learning_rate)*self.W_out + self.learning_rate*np.dot(np.linalg.pinv(extended_states[transient:, :]), self.inverse_out_activation(teachers_scaled[transient:, :])).T
